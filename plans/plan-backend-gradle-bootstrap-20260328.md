...
**Source of Truth:** GitHub issue #2 plus ADR-001 project structure baseline.
...
`apps/api` and `modules/common` backend modules. Excludes business logic,
database setup, a worker placeholder or runtime, and any frontend or infra
work.
...
and dependency resolution. Do not add bootstrap application source code yet;
verification should stop at the Gradle skeleton level for this first pass.
...
- Keep the plugin build simpler than the fully decomposed `mixin/` and
  `convention/` split used by `modern-gradle-template-simplified`, because
  issue #2 explicitly asks for one `backend` plugin module plus one
  `version-catalog` module.
...
- [x] **Step 1:** Run `gradle init` inside `backend/` to generate the wrapper
  and Kotlin DSL root files, using it only as a bootstrap step rather than the
  final layout generator.
- [x] **Step 2:** Replace any irrelevant generated sample project content with a
  lightweight root build that only declares globally shared plugin aliases and
  leaves real configuration to convention plugins.
...
- [x] **Step 4:** Populate `backend/gradle/libs.versions.toml` with Java 25,
  ...
  Develocity, Foojay, and any plugin dependencies or BOM coordinates needed by
...
Create the internal Gradle build that houses reusable precompiled script
plugins and the generated version-catalog access layer.
...
- Create: `backend/gradle/plugins/backend/src/main/kotlin/my.jvm-common.gradle.kts`
- Create: `backend/gradle/plugins/backend/src/main/kotlin/my.lib.gradle.kts`
- Create: `backend/gradle/plugins/backend/src/main/kotlin/my.spring-app.gradle.kts`
...
- Run: `cd backend && ./gradlew :apps:api:tasks --all`
- Run: `cd backend && ./gradlew :apps:api:dependencies --configuration implementation`
- Expect: Both modules configure successfully as empty Gradle skeletons, the
  API module uses the custom app plugin, and the common module resolves the
  Spring BOM through its `implementation` configuration.
...
### Task 5: Document the Bootstrap Decisions

#### Intent:

Leave enough project-local documentation that the Gradle layout remains obvious
after this first backend setup lands.
...
- [ ] **Step 1:** Document the backend folder layout and the roles of the root
  build, the version catalog, and the internal plugin build.
- [ ] **Step 2:** Document the three precompiled backend plugins and when a
  module should use each one.
- [ ] **Step 3:** Document the main verification and bootstrap commands, such as
  `./gradlew projects` and the module build commands used during setup.

#### Verification:

- Expect: A new contributor can open `backend/README.md` and understand how the
  backend Gradle structure is organized and where future modules should plug in.

#### Notes:

- Keep the README focused on project-specific conventions rather than generic
  Gradle explanations.
...
| --- | --- | --- |
| ![ADR-001](docs/adr/0001-project-structure-baseline.md:1:110) | Accepted project structure baseline defining backend as a split-root Gradle workspace with `apps/api`, `apps/worker`, and shared modules. | Must Read |
