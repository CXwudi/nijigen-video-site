...
- [ ] **Step 1:** Check the current latest GA or project-preferred versions for Node, pnpm, TanStack Start, TanStack Router, React, Tailwind CSS, daisyUI, TanStack Query, Zustand, Vite, Vitest, Oxlint, Oxfmt, TypeScript, and related React/Vite plugins.
- [ ] **Step 2:** Decide whether the scaffold should be generated by an official TanStack command or built manually from the smallest TanStack Start template; prefer the official generator if it supports the requested stack cleanly.
- [ ] **Step 3:** Record any version or scaffold assumptions in the implementation notes or the PR description, especially if "latest GA" conflicts with the repository's root `mise.toml` `node = "lts"` behavior.

#### 1.4 Verification:

- Run: `mise current node pnpm`
- Run: `pnpm --version`
