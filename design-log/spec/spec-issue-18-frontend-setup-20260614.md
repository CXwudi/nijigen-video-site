...
Application dependency versions should be selected from the current latest GA
versions at implementation time. Node.js is the exception: pin major version 24
across root mise tooling and every frontend Docker stage. This keeps host
development, Docker development, builds, and production runtime on the same
Node.js release line without allowing an `lts` alias to advance them
independently. pnpm remains independently updateable through its `latest` tag.
...
The official pnpm image should provide the current stable pnpm release, and
pnpm should install the latest available Node.js 24 release through
`pnpm runtime set`. The generic `frontend-pnpm-base` Docker stage owns only the
frontend workspace toolchain; the `web-pnpm-base` Docker target remains
web-specific and prepares the web dependency store for local development and
builds.
...
The runtime stage should use BellSoft's hardened Node.js 24 glibc image because
the packaged TanStack Start artifact needs Node.js but does not require pnpm.
The major-only tag should receive compatible Node.js 24 patch updates without
automatically advancing to a later Node.js major. The glibc variant avoids a
build/runtime libc mismatch if the application later includes native Node.js
modules. It also aligns the frontend runtime image provider with the existing
BellSoft hardened backend images.

The final artifact path and runtime command should be verified against the
actual TanStack Start output during implementation. The exposed container port
should also follow the default port generated by the TanStack Start setup. The
intent is stable even if the exact output path, port, or command needs
adjustment.
...
### Floating Node.js LTS aliases

This was not chosen because `lts` can move host tooling, Docker build stages,
and the production runtime to a new major at different times. Pinning major
version 24 keeps those environments aligned while still receiving patch
updates.

### Docker Official Node.js runtime image

`node:24-slim` would provide a familiar glibc-based runtime, but it was not
chosen because the backend already establishes BellSoft hardened images as the
project's runtime-image convention.
...
- production hardening beyond selecting the BellSoft hardened Node.js runtime
  image;
...
- The exact current latest GA dependency versions must be checked during
  implementation, especially for TanStack Start, daisyUI, Tailwind CSS,
  Vitest, Oxlint, and Oxfmt.
- A future Node.js major upgrade must update root mise tooling, the pnpm runtime
  installation, and the hardened runtime image together.
...
- `mise current node`, confirming the selected host runtime is Node.js 24;
...
- Node.js 24 in both the frontend build stage and final runtime image;
...
| [../../mise.toml](../../mise.toml) | Root tool configuration that pins the frontend Node.js major version. | Important |
...
| [BellSoft hardened image documentation](https://docs.bell-sw.com/alpaquita-linux/latest/how-to/images-getting-started-guide/#hardened-nodejs) | Defines hardened Node.js tags, variants, and libc choices. | Must Read |
