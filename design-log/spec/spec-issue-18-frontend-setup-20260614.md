...
The `web` local service will be behind a Compose profile named `web`. `mise` is the blessed task runner, so it is acceptable if raw `docker compose up -d` from `frontend/docker/` does not start the complete frontend stack.
...
```dockerfile
FROM node:<current-ga-version>-slim AS web-node-base
ENV PNPM_HOME="/pnpm"
...
The exact Node image tag should be selected at implementation time from the current latest GA version that fits the repository's support policy.
...
- `up`: start the full Docker-first frontend local stack using the `web` profile;
- `up-backend`: start the API and backend dependencies without the `web` profile;
- `down`: stop the frontend local stack and remove orphans;
- `config`: render the full local config with the `web` profile;
- `config-check`: validate the full local config with the `web` profile;
...
- `mise //frontend/docker:up`, confirming the web service starts through the `web` profile;
